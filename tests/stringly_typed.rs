// TODO: ensure no-std still works
// TODO: provide a premade one of these in the library itself, so that people can use it with no trouble?
// THOUGHT: Source must always be an `Option`. This seems to force the `from()`

mod message_only {
    use snafu::{snafux, Snafu};

    #[derive(Debug, Snafu)]
    enum Error {
        #[snafu(other, display("{}", message))]
        Other { message: String },
    }

    // THOUGHT: Allow identify message field?

    type Result<T, E = Error> = std::result::Result<T, E>;

    #[test]
    fn implements_error() {
        fn check<T: std::error::Error>() {}
        check::<Error>();
    }

    #[test]
    fn can_use_a_formatted_string() {
        fn exercise(success: bool) -> Result<i32> {
            if !success {
                snafux!("This is a code {} error", 42);
            }

            Ok(1)
        }

        assert!(matches!(exercise(true), Ok(1)));
        let e = exercise(false).unwrap_err();
        assert_eq!("This is a code 42 error", e.to_string());
    }
}

// THOUGHT: Must it be boxed trait object?
// No, it *can* be a fixed type, but that's very limiting

mod message_and_source {
    use snafu::{ensure, snafux, ResultExt, Snafu};

    #[derive(Debug, Snafu)]
    struct UnderlyingError;

    fn underlying(success: bool) -> Result<i32, UnderlyingError> {
        ensure!(success, UnderlyingContext);
        Ok(1)
    }

    #[derive(Debug, Snafu)]
    enum Error {
        // THOUGHT: Should display automatically do message?
        #[snafu(other, display("{}", message))]
        Other {
            #[snafu(source(from(Box<dyn std::error::Error>, Some)))] // FIXME
            source: Option<Box<dyn std::error::Error>>,
            message: String,
        },
    }

    type Result<T, E = Error> = std::result::Result<T, E>;

    #[test]
    fn implements_error() {
        fn check<T: std::error::Error>() {}
        check::<Error>();
    }

    #[test]
    fn can_use_a_formatted_string_via_macro() {
        fn exercise(success: bool) -> Result<i32> {
            let v = snafux!(underlying(success), "Something else happened {}", 42);
            Ok(v + 1)
        }

        assert!(matches!(exercise(true), Ok(2)));
        let e = exercise(false).unwrap_err();
        assert_eq!("Something else happened 42", e.to_string());
    }

    #[test]
    fn can_use_a_formatted_string_via_trait() {
        fn exercise(success: bool) -> Result<i32> {
            let v =
                underlying(success).other_context(|_| format!("Something else happened {}", 42))?;
            Ok(v + 1)
        }

        assert!(matches!(exercise(true), Ok(2)));
        let e = exercise(false).unwrap_err();
        assert_eq!("Something else happened 42", e.to_string());
    }

    #[test]
    fn can_access_the_cause() {
        use std::error::Error as _;

        fn exercise(success: bool) -> Result<i32> {
            let v = snafux!(underlying(success), "Something else happened {}", 42);
            Ok(v + 1)
        }

        let e = exercise(false).unwrap_err();
        let src = e.source().expect("Must have a source");
        let src = src.downcast_ref::<UnderlyingError>();
        assert!(src.is_some());
    }

    #[test]
    fn does_not_need_a_cause() {
        use std::error::Error as _;

        fn exercise(success: bool) -> Result<i32> {
            if !success {
                snafux!("I caused a problem {}", 42);
            }
            Ok(1)
        }

        let e = exercise(false).unwrap_err();
        assert!(e.source().is_none());
    }

    #[test]
    fn can_be_recursive() {
        use std::error::Error as _;

        fn inner(success: bool) -> Result<i32> {
            if !success {
                snafux!("Inner error");
            }
            Ok(1)
        }

        fn outer(success: bool) -> Result<i32> {
            let v = snafux!(inner(success), "Outer error");
            Ok(v + 1)
        }

        assert!(matches!(outer(true), Ok(2)));
        let outer_error = outer(false).unwrap_err();
        let inner_error = outer_error.source().expect("Must have a source");
        assert!(inner_error.downcast_ref::<Error>().is_some());
        assert!(inner_error.source().is_none());
    }
}

mod message_source_and_backtrace {
    use snafu::{ensure, snafux, Backtrace, Snafu};

    #[derive(Debug, Snafu)]
    struct UnderlyingError;

    fn underlying(success: bool) -> Result<i32, UnderlyingError> {
        ensure!(success, UnderlyingContext);
        Ok(1)
    }

    #[derive(Debug, Snafu)]
    enum Error {
        #[snafu(other, display("{}", message))]
        Other {
            #[snafu(source(from(Box<dyn std::error::Error>, Some)))] // FIXME
            source: Option<Box<dyn std::error::Error>>,
            message: String,
            backtrace: Backtrace,
        },
    }

    type Result<T, E = Error> = std::result::Result<T, E>;

    #[test]
    fn implements_error() {
        fn check<T: std::error::Error>() {}
        check::<Error>();
    }

    #[test]
    fn can_use_a_formatted_string() {
        fn exercise(success: bool) -> Result<i32> {
            let v = snafux!(underlying(success), "Something else happened {}", 42);
            Ok(v + 1)
        }

        assert!(matches!(exercise(true), Ok(2)));
        let e = exercise(false).unwrap_err();
        assert_eq!("Something else happened 42", e.to_string());
    }

    #[test]
    fn has_a_backtrace() {
        use snafu::ErrorCompat;

        fn exercise(success: bool) -> Result<i32> {
            let v = snafux!(underlying(success), "Something else happened {}", 42);
            Ok(v + 1)
        }

        let e = exercise(false).unwrap_err();
        let bt = ErrorCompat::backtrace(&e).expect("Must have a backtrace");
        assert_eq!("disabled backtrace", bt.to_string());
    }
}

mod struck {
    mod message_source_and_backtrace {
        use snafu::{snafux, Backtrace, Snafu};

        #[derive(Debug, Snafu)]
        #[snafu(other, display("{}", message))]
        struct Error {
            #[snafu(source(from(Box<dyn std::error::Error>, Some)))] // FIXME
            source: Option<Box<dyn std::error::Error>>,
            message: String,
            backtrace: Backtrace,
        }

        type Result<T, E = Error> = std::result::Result<T, E>;

        #[test]
        fn implements_error() {
            fn check<T: std::error::Error>() {}
            check::<Error>();
        }

        #[test]
        fn can_use_a_formatted_string() {
            fn inner(success: bool) -> Result<i32> {
                if !success {
                    snafux!("inner went {}", "bang");
                }
                Ok(1)
            }

            fn outer(success: bool) -> Result<i32> {
                let v = snafux!(inner(success), "Something else happened {}", 42);
                Ok(v + 1)
            }

            assert!(matches!(outer(true), Ok(2)));
            let e = outer(false).unwrap_err();
            assert_eq!("Something else happened 42", e.to_string());
        }
    }
}
